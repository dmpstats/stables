...
prepObsPPM <- function(
  ...
  if (!inherits(observations, "sf") || !all(sf::st_geometry_type(observations) %in% c("POINT", "MULTIPOINT"))) {
    cli::cli_abort("observations must be an sf dataframe with POINT or MULTIPOINT geometries.")
  }
  ...
  if (!inherits(tracks, "sf") || !all(sf::st_geometry_type(tracks) %in% c("LINESTRING", "MULTILINESTRING", "POLYGON", "MULTIPOLYGON"))) {
    cli::cli_abort("tracks must be an sf dataframe with LINESTRING or POLYGON geometries.")
  }
  ...
  if (targetBehaviour != "All" && !"Behaviour" %in% colnames(observations)) {
    cli::cli_abort("The observations data must contain a 'Behaviour' column to filter by behaviour.")
  }
  # Check that the provided species is within the observations data
  if (!targetSpecies %in% unique(observations$Species)) {
    cli::cli_abort("The specified species '{targetSpecies}' is not found in the observations data.")
  }
  # If behaviour is not "All", check it's in the data
  if (targetBehaviour != "All" &&
    !any(stringr::str_detect(tolower(observations$Behaviour), tolower(targetBehaviour)))) {
    cli::cli_abort("The specified behaviour '{targetBehaviour}' is not found in the observations data.")
  }

  # If Behaviour is provided, we list all the behaviours that string-match
  # so that the user can identify errors
  if (targetBehaviour != "All") {
    matching_behaviours <- unique(observations$Behaviour[stringr::str_detect(tolower(observations$Behaviour), tolower(targetBehaviour))])
  ...
  filtered_obs <- observations %>%
    dplyr::filter(Species == targetSpecies) %>% {
      if (targetBehaviour != "All") dplyr::filter(., stringr::str_detect(tolower(Behaviour), tolower(targetBehaviour))) else .
    }
  # Report how many observations remain after filtering
  n_filtered <- nrow(filtered_obs)
  cli::cli_alert_info("{n_filtered} observations remain after filtering for species '{targetSpecies}' and behaviour '{targetBehaviour}'.")
  if (n_filtered == 0) {
    cli::cli_abort("No observations remain after filtering. Please check your species and behaviour filters.")
  }

  # Check that all observations are within survey tolerance of tracks or polygons
  track_buffer <- sf::st_buffer(tracks, dist = units::set_units(survey_tolerance, "meters"))
  obs_within_buffer <- sf::st_within(filtered_obs, sf::st_union(track_buffer), sparse = FALSE)
  if (any(!obs_within_buffer)) {
    n_outside <- sum(!obs_within_buffer)
    percent_outside <- round((n_outside / nrow(filtered_obs)) * 100, 2)
    cli::cli_warn("{n_outside} observations ({percent_outside}% of total) are outside the survey tolerance of {survey_tolerance} metres from the tracks/polygons. They will be removed.")
